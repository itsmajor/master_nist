...
def renyi(p1, p2, a):
    """Compute the Renyi divergence between two distributions.
    ...
    try:
        # It is possible to do addition in the logspace with np.logaddexp.
        s = sum(
            [p1[v] * float(p1[v] / p2[v]) ** (a - 1) for v in p1 if p1[v] > 0])
...
def renyi_bound(ln_pr, r, a):
    """Compute an upper bound on probability via Renyi divergence.
    ...
    Returns:
      Natural logarithm on an upper bound on the probability of the same event
      under p1.
    """
    if a == float('inf'):
        return ln_pr + r
...
def opt_renyi_bound(ln_pr, p1, p2, n):
    """Find the optimal Renyi order and the corresponding bound.

    Given two distributions p1 and p2, and an event of probability exp(ln_pr)
    under p2^n (n-fold direct product), find the optimal Renyi order so that
    the upper bound via the Renyi divergence on the event's probability under
    p1^n is minimized.
