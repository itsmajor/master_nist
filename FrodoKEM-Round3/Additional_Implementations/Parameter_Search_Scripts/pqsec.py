...
def dual_cost(q, n, m, s, b, cost_svp=svp_classical, verbose=False):
    ...
    if verbose:
        ...
        print('log2(epsilon) = {}, {} log2 nvector per run'.format(
...
def main():
    print('Frodo-640 parameters (single instance, dual attack, '
          'before reduction)')
    q, n, samples, sigma = 2 ** 15, 640, 640 + 8, 2.8
    m, b, c = optimize_attack(
        q, n, samples, sigma, cost_attack=dual_cost, cost_svp=svp_classical)
    print('m = {}, b = {}, C = {:.2f}'.format(m, b, c))

    m, b, c = optimize_attack(
        q, n, samples, sigma, cost_attack=dual_cost, cost_svp=svp_quantum)
    print('m = {}, b = {}, Q = {:.2f}'.format(m, b, c))

    m, b, c = optimize_attack(
        q, n, samples, sigma, cost_attack=dual_cost, cost_svp=svp_plausible)
