 ...
 * @param[in] cdw Array of size VEC_N1_BYTESIZE storing the received vector
 */
void compute_syndromes(ap_uint8 syndromes[2 * PARAM_DELTA], ap_uint8 cdw[VEC_N1_BYTESIZE]) {
    ap_uint6 i;
    ap_uint7 j;
	ap_uint8 syndrome_local;
	synd1:for (i = 0 ; i < 2 * PARAM_DELTA ; ++i) {
		syndrome_local = syndromes[i];
        synd2:for (j = 1 ; j < VEC_N1_BYTESIZE ; ++j) {
			syndrome_local ^= gf_mul(cdw[j], alpha_ij_pow[i][j-1]);
		}
		syndromes[i] = syndrome_local ^ cdw[0];
	}
}
 ...
 * @param[out] sigma Array of size (at most) PARAM_DELTA receiving the ELP
 * @param[in] syndromes Array of size (at most) 2*PARAM_DELTA storing the syndromes
 */
ap_uint8 compute_elp(ap_uint8 sigma[1 << PARAM_FFT], const ap_uint8 syndromes[2 * PARAM_DELTA]) {

	ap_uint8 deg_sigma = 0;
	ap_uint8 deg_sigma_p = 0;
	ap_uint8 sigma_local, X_sigma_p_local, X_sigma_p_prev;
	ap_uint8 deg_sigma_copy;
	ap_uint8 X_sigma_p[PARAM_DELTA + 1];
	ap_int8 pp = -1;
	ap_uint8 d_p = 1;
	const ap_uint8 masks[2] = {0,(ap_uint8)-1};
	ap_uint8 retrieved_mask;

	ap_uint8 d = syndromes[0];

    ap_uint8 mu, mu_add, deg_X;
	ap_uint16 deg_X_sigma_p;
    ap_uint8 i;
    ap_uint8 dd;
	...
	// Constant time loop
    elp1:for (mu = 0 ; mu < (2 * PARAM_DELTA)  ; ++mu) {
		mu_add = mu + 1;

        deg_sigma_copy = deg_sigma;
		dd = gf_mul((ap_uint8)d, gf_inverse(d_p));

		deg_X = (ap_uint8)(mu - pp);
		deg_X_sigma_p = (ap_uint16)deg_X + (ap_uint16)deg_sigma_p;

		// retrieved_mask = 0xff if the deg_sigma increased and 0 otherwise
		retrieved_mask = masks[(d > 0) & (deg_X_sigma_p > (ap_uint16)deg_sigma)];
		deg_sigma ^= retrieved_mask & ((ap_uint8)deg_X_sigma_p^deg_sigma);
		pp ^= (ap_int8)retrieved_mask & ((ap_int8)mu^pp);
		d_p ^= retrieved_mask & (d^d_p);

		d = syndromes[mu_add];
		sigma_local = sigma[0];
		X_sigma_p_prev = X_sigma_p[0];

		// Constant time loop
		elp1_2:for (i = 1 ;  i <= PARAM_DELTA ; ++i) {
		    X_sigma_p_local = masks[i <= mu_add] & X_sigma_p[i];
			sigma[i] ^= gf_mul(dd, X_sigma_p_local);
			X_sigma_p_local = X_sigma_p_prev ^ (retrieved_mask & (sigma_local ^ X_sigma_p_prev)) ;
			X_sigma_p[i] = X_sigma_p_local;
			sigma_local = sigma[i];
			X_sigma_p_prev = X_sigma_p[i];
		}
		deg_sigma_p ^= retrieved_mask & (deg_sigma_copy^deg_sigma_p);

		// Constant time loop
        elp1_4:for (i = 1 ;  i <= PARAM_DELTA ; ++i) {
		    d ^= masks[i <= mu_add] & gf_mul(sigma[i], syndromes[mu_add - i]);
		}
	}
 ...
 * @param[in] syndromes Array of 2 * PARAM_DELTA bytes storing the syndromes
 */
void compute_z_poly(ap_uint8 z[PARAM_DELTA + 1], const ap_uint8 sigma[1 << PARAM_FFT], const ap_uint8 degree, const ap_uint8 syndromes[2 * PARAM_DELTA]) {
	...
	z[1] = sigma[1] ^ syndromes[0];
    z_poly2:for (i = 2 ; i <= PARAM_DELTA ; ++i) {
	    retrieved_mask = masks[i < degree - 1];
        z_local = retrieved_mask & (sigma[i]^syndromes[i-1]);
        z_poly3:for (j = 1 ; j < i ; ++j) {
			z[i] ^= retrieved_mask & (z[i] ^ z_local ^ gf_mul(sigma[j], syndromes[i - j - 1]));
		}
	}
}
 ...
 * @param[in] z_degree Degree of polynomial z(x)
 * @param[in] error Array of VEC_N1_BYTESIZE bytes storing the error
 */
void compute_error_values(ap_uint8 error_values[PARAM_DELTA], ap_uint8 z[PARAM_DELTA + 1], ap_uint8 error[VEC_N1_BYTESIZE]) {
	ap_uint8 beta_j[PARAM_DELTA];
	ap_uint8 e_j[PARAM_DELTA];

	ap_uint11 delta_counter = 0;
	
    ap_uint6 i;
    ap_uint5 j, k;

    ap_uint8 inverse_power_j, error_values_a, error_values_b, inverse;
    const ap_uint8 masks[2] = {0,(ap_uint8)-1};
    ap_uint8 retrieved_mask;

	memset_beta_j : for (j = 0; j < PARAM_DELTA; ++j) {
		beta_j[j] = 0;
	}

    // Compute the beta_{j_i} page 31 of the documentation
    error_values1:for (i = 0 ; i < VEC_N1_BYTESIZE ; i++) {
        retrieved_mask = masks[(error[i] != 0) & (delta_counter < PARAM_DELTA)];
        beta_j[delta_counter] += retrieved_mask & exp_array[i];
        delta_counter += retrieved_mask & 1;
	}

	// Compute the e_{j_i} page 31 of the documentation
    error_values3:for (i = 0 ; i < PARAM_DELTA ; ++i) {
		error_values_a = 1;
		error_values_b = 1;
		inverse = gf_inverse(beta_j[i]);
		inverse_power_j = 1;

        error_values4:for (j = 1 ; j <= PARAM_DELTA ; ++j) {
			inverse_power_j = gf_mul(inverse_power_j, inverse);
			error_values_a ^= gf_mul(inverse_power_j , z[j]);
		}
        error_values5:for (k = 1 ; k < PARAM_DELTA ; ++k) {
			error_values_b = gf_mul(error_values_b, (1 ^ gf_mul(inverse, beta_j[(i+k) % PARAM_DELTA])));
		}
		e_j[i] ^= masks[i < delta_counter] & (e_j[i]^gf_mul(error_values_a,gf_inverse(error_values_b)));

	}

	// Place the delta e_{j_i} values at the right coordinates of the output vector
	delta_counter = 0;
    error_values6:for (i = 0 ; i < VEC_N1_BYTESIZE ; ++i) {
        retrieved_mask = masks[error[i] != 0 & delta_counter < PARAM_DELTA];
        error_values[delta_counter] += retrieved_mask & e_j[i];
        delta_counter += retrieved_mask & 1;
	}
}
 ...
 *
 * @param[out] cdw Array of VEC_N1_BYTESIZE elements receiving the corrected vector
 * @param[in] error_values Array of VEC_N_BYTESIZE elements storing the error values
 */
void correct_errors(ap_uint8 cdw[VEC_N1_BYTESIZE], const ap_uint8 error_values[VEC_N1_BYTESIZE]) {
	ap_uint6 i;
    correct_err:for (i = 0 ; i < VEC_N1_BYTESIZE ; ++i) {
 ...
 *
 * @param[out] msg Array of size VEC_K_BYTESIZE receiving the decoded message
 * @param[in] cdw Array of size VEC_N1_BYTESIZE storing the received word
 */
void reed_solomon_decode(ap_uint8 msg[VEC_K_BYTESIZE], ap_uint8 cdw[VEC_N1_BYTESIZE]) {

	ap_uint8 syndromes[2 * PARAM_DELTA];
	ap_uint8 sigma[1 << PARAM_FFT];
	ap_uint8 error[1 << PARAM_M];
	ap_uint8 z[VEC_N1_BYTESIZE];
	ap_uint8 error_values[VEC_N1_BYTESIZE];

	ap_uint9 i;
	ap_uint8 deg;

	// Array initialization
	vector_init(syndromes, (2 * PARAM_DELTA));
	vector_init(sigma, (1 << PARAM_FFT));
	vector_init(z, VEC_N1_BYTESIZE);
	vector_init(error_values, VEC_N1_BYTESIZE);
    ...
    rs_decode:for(i = 0; i < PARAM_K; ++i) {
        msg[i] = cdw[i + (PARAM_G - 1)];
    }
    
#ifdef VERBOSE
	printf("\n\nThe syndromes: ");
	for (size_t i = 0 ; i < 2*PARAM_DELTA ; ++i) {
		printf("%u ", (uint8_t)syndromes[i]);
	}
	printf("\n\nThe error locator polynomial: sigma(x) = ");
	bool first_coeff = true;
	if (sigma[0]) {
		printf("%u", (uint8_t)sigma[0]);
		first_coeff = false;
	}
	...
	size_t j = 0;
	for (size_t i = 0 ; i < VEC_N1_BYTESIZE ; ++i) {
		if(error[i]){
			printf("(%zu, %d) ", i, (uint8_t)error_values[j]);
			j++;
		}
	}
	printf("\n");
