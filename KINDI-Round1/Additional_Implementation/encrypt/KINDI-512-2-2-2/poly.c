...
void poly_setrandom_rsec(poly_d *r,poly_d *e, uint8_t *gamma) {
	...
	int bufferlen = 2*KINDI_CPA_L*KINDI_CPA_N*(KINDI_CPA_LOG2RSEC+1)/8;
	uint8_t *buffer = malloc(bufferlen);
	kindi_crypto_stream(buffer,bufferlen,gamma,KINDI_CPA_SEEDSIZE);
	int i;
	uint64_t offset =0 ;
	for(i=0;i<KINDI_CPA_L;i++){

		poly_neg_frombytes_bitlen(r[i],buffer+offset,KINDI_CPA_LOG2RSEC+1);
		offset+= KINDI_CPA_N*(KINDI_CPA_LOG2RSEC+1)/8;
		poly_neg_frombytes_bitlen(e[i],buffer+offset,KINDI_CPA_LOG2RSEC+1);
		offset+= KINDI_CPA_N*(KINDI_CPA_LOG2RSEC+1)/8;
	}
...
void poly_gen_matrix(poly_d **A, uint8_t * seed) {
	...
	uint32_t tmp;
	int buffer_size = KINDI_CPA_L * KINDI_CPA_L * KINDI_CPA_N * KINDI_CPA_BYTESLOGQ;
	uint8_t *buffer = (uint8_t *) malloc(buffer_size);

	kindi_crypto_stream(buffer, buffer_size, seed, KINDI_CPA_SEEDSIZE);
	int a, x, i, y;
	int pos = 0;
	for (a = 0; a < KINDI_CPA_L; a++)
		for (x = 0; x < KINDI_CPA_L; x++)
			for (i = 0; i < KINDI_CPA_N; i++) {
				tmp = 0;
				for (y = 0; y < KINDI_CPA_BYTESLOGQ; y++)
					tmp += (int) buffer[pos++] << y * 8;

				A[a][x][i] = tmp & (KINDI_CPA_Q - 1);
...
void poly_coeffreduce_center(poly_d r) {
	...
	for (i = 0; i < KINDI_CPA_N; i++) {
		r[i] = r[i] & (KINDI_CPA_Q - 1);
		if (r[i] > (KINDI_CPA_Q - 1) / 2)
			r[i] = r[i] - KINDI_CPA_Q;

		else if (r[i] < -(KINDI_CPA_Q - 1) / 2)
			r[i] = r[i] + KINDI_CPA_Q;
	}
