...
void kindi_cpa_keygen(kindi_pk *pk, poly_d *sk_r) {
	...
	A = (poly_d**) memalign(32, KINDI_CPA_L * sizeof(poly_d*));
	e = (poly_d*) memalign(32, KINDI_CPA_L * sizeof(poly_d));
	for (i = 0; i < KINDI_CPA_L; i++)
		A[i] = (poly_d*) memalign(32, KINDI_CPA_L * sizeof(poly_d));
	...
	r_real = (poly_f*) memalign(32, KINDI_CPA_L * sizeof(poly_f));
	r_imag = (poly_f*) memalign(32, KINDI_CPA_L * sizeof(poly_f));
	for (i = 0; i < KINDI_CPA_L; i++)
...
void kindi_cpa_encrypt(kindi_pk *pk, const uint8_t *message_in,
	...
	s = (poly_d *) memalign(32, KINDI_CPA_L * sizeof(poly_d));
	u_encoded = (poly_d*) memalign(32, (KINDI_CPA_L + 1) * sizeof(poly_d));
	A = (poly_d**) memalign(32, KINDI_CPA_L * sizeof(poly_d*));
	for (i = 0; i < KINDI_CPA_L; i++)
		A[i] = (poly_d*) memalign(32, KINDI_CPA_L * sizeof(poly_d));

	poly_f *s_real, *s_imag;
	s_real = (poly_f*) memalign(32, KINDI_CPA_L * sizeof(poly_f));
	s_imag = (poly_f*) memalign(32, KINDI_CPA_L * sizeof(poly_f));
...
void kindi_cpa_decrypt(poly_d *sk_r, kindi_pk *pk, poly_d *cipher,
	...
	u_rec = (poly_d *) memalign(32, (KINDI_CPA_L + 1) * sizeof(poly_d));
	s_rec = (poly_d *) memalign(32, KINDI_CPA_L * sizeof(poly_d));
	A = (poly_d**) memalign(32, KINDI_CPA_L * sizeof(poly_d *));
	for (i = 0; i < KINDI_CPA_L; i++)
		A[i] = (poly_d*) memalign(32, KINDI_CPA_L * sizeof(poly_d));

	poly_f *s_rec_real, *s_rec_imag;
	s_rec_real = (poly_f*) memalign(32, KINDI_CPA_L * sizeof(poly_f));
	s_rec_imag = (poly_f*) memalign(32, KINDI_CPA_L * sizeof(poly_f));
