...
#define _mm256_mulcmplx_pd(result_acmbd, result_adpbc, \
							vec1_a,vec1_b,\
							vec2_c,vec2_d)\
   { temp_ad = _mm256_mul_pd(vec1_a,vec2_d);\
	vec1_a= _mm256_mul_pd(vec1_a,vec2_c); \
	temp_bd=_mm256_mul_pd(vec1_b,vec2_d);\
	result_adpbc = _mm256_mul_pd(vec1_b,vec2_c);\
	result_acmbd= _mm256_sub_pd(vec1_a,temp_bd); \
	result_adpbc= _mm256_add_pd(result_adpbc,temp_ad);}
