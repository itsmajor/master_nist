...
void kindi_kem_encaps(kindi_pk *pk_p, uint8_t *ct, uint8_t *ss) {
	...
	int hash_c_inlen = KINDI_KEM_CIPHERBYTES - KINDI_KEM_HASHSIZE;
	uint8_t *hash_c_in = malloc(hash_c_inlen);
	memcpy(hash_c_in, ct, KINDI_KEM_CIPHERBYTES - KINDI_KEM_HASHSIZE);

	uint8_t *hash_c_out = malloc(4*KINDI_KEM_HASHSIZE);

	// H(c) with shake4x
	shake4x_hash(hash_c_out, 4*KINDI_KEM_HASHSIZE, hash_c_in, hash_c_inlen);
	int hash_s1c_inlen = 4*KINDI_KEM_HASHSIZE + KINDI_KEM_S1SIZE;
	uint8_t *hash_s1c_in = malloc(hash_s1c_inlen);
	memcpy(hash_s1c_in, hash_c_out, 4*KINDI_KEM_HASHSIZE);
	memcpy(hash_s1c_in + 4*KINDI_KEM_HASHSIZE, s1, KINDI_KEM_S1SIZE);

	// H'(s1,H(c))
	crypto_hash(ss, KINDI_KEM_HASHSIZE, hash_s1c_in, hash_s1c_inlen);
	free(hash_c_in);
	free(hash_c_out);
	free(hash_s1c_in);
...
void kindi_kem_decaps(poly_d *sk_p, kindi_pk *pk_p, poly_d *cipher, uint8_t *d,
	...
	// shake4x(c) = out1,..., out4  with shake4x
	shake4x_hash(hash_c_out, 4*KINDI_KEM_HASHSIZE, hash_c_in, hash_c_inlen);
	int hash_s1c_inlen = 4*KINDI_KEM_HASHSIZE + KINDI_KEM_S1SIZE;
	uint8_t *hash_s1c_in = malloc(hash_s1c_inlen);
	memcpy(hash_s1c_in, hash_c_out, 4*KINDI_KEM_HASHSIZE);
	...
	if (memcmp(d, d_rec, KINDI_KEM_HASHSIZE) == 0
		...
		// return H'(s1',(\vec{c},H(c)))
		memcpy(hash_s1c_in + 4*KINDI_KEM_HASHSIZE, s1_rec, KINDI_KEM_S1SIZE);
	} else {
		uint8_t *uniform_rand = malloc(KINDI_KEM_S1SIZE);
		randombytes(uniform_rand, KINDI_KEM_S1SIZE);

		// return H'(random,(\vec{c},c))
		memcpy(hash_s1c_in + 4*KINDI_KEM_HASHSIZE, uniform_rand,
				KINDI_KEM_S1SIZE);
		free(uniform_rand);
	}
