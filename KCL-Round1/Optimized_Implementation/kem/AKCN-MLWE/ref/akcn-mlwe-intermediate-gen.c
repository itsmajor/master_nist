...
fprintBstr(fp_dec_vector, "sk = ", sk, CRYPTO_SECRETKEYBYTES);
	...
	// unsigned char * X1_string = sk;

	Polynomial Sigma1;

	Polynomial Y2[MLWE_ELL];
	unsigned int Signal[MLWE_N];
	// unsigned char * Y2_string, * Signal_string;

	unsigned int Key1[MLWE_N];
	unsigned char * Key1_string = ss;

	/* unpack*/

		// unpack ct = (Y2, Signal)
		unpack_ct(Y2, Signal, ct);

		// unpack sk = X1
		unpack_sk(X1, sk);

	/* computation in Decaps() */
		// compute Sigma1
	...
	for(i=0; i<MLWE_ELL; i++)
		{
			...
			// generate E1[i]
			Get_small_poly(&E1_poly, noise_seed, nonce++);	
			Poly_add_then_truncate(Y1+i, &E1_poly, MLWE_T);
		}
	}}
...
printpolyvec(Y1, fp_keypair_vector);
	...
	/* pack */
		// note: X1 has already been packed previously

		// pack pk = (Y1, seed).
		// note: matrix_seed has already been packed
		pack_truncated_vector(Y1_string, Y1);
...
printpolymatrix(X2, fp_enc_vector);
		...
		{{// compute truncated(Y2) and Sigma2
...
fprintf(fp_enc_vector, "\nPolynomial vector Y1 after Get_small_poly will be:  \n"  );
printpolymatrix(Y1, fp_enc_vector);
		...
		for(i=0; i<MLWE_ELL; i++)
		{
			...
			Get_small_poly(&error_poly, noise_seed, nonce++); // generate E2[i]
			Poly_add_then_truncate(Y2+i, &error_poly, MLWE_T);
		}
		...
		Poly_post_NTT_computation(&Sigma2);
	        ...
	        Get_small_poly(&error_poly, noise_seed, nonce++);// generate E_sigma
		(&Sigma2, &error_poly, 0);

fprintf(fp_enc_vector, "\nPolynomial vector Sigma2 after Poly_add_then_truncate is:  \n"  );
printpolymatrix(Y2, fp_enc_vector);

		}}

		/* Finally, compute (Key2, v) <- Con(Sigma2) */
			// (*prg)(noise_seed, MLWE_N/8, noise_seed, NOISE_SEED_BYTES);
	...
	{{ // (Key2, v) <- Con(Sigma2)
		...
		for(i=0, index=0; i<MLWE_N/8; i++)
		{
			temp = noise_seed[i];
			for(j=0; j<8; j++)
			{
				Key2[index++] = (temp & 0x01);
				temp = (temp>>1);
			}
		}
	...
	// packing
		// pack ct = (Y2, Signal)
		// pack_truncated_vector(Y2_string, Y2);
		// pack_signal(Signal_string, Signal);
		pack_ct(ct, Y2, Signal);

		// pack Key2
		pack_key(Key2_string, Key2);
