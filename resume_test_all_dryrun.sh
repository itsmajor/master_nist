#!/bin/bash
# resume_test_all_dry2.sh
# Dry‑Run-Version zum Fortsetzen eines abgebrochenen Testlaufs.
...
echo "=== RESUME TEST RUN (DRY RUN) ==="
echo "Prüfe, ob das Verzeichnis 'testresult' existiert..."
if [ ! -d "testresult" ]; then
  echo "-> 'testresult' existiert NICHT. Kein Testlauf in Verlaufe."
  exit 0
fi

LOGFILE="testresult/verifyresult.log"
if [ ! -f "$LOGFILE" ]; then
  echo "-> Logdatei $LOGFILE nicht gefunden. Starte initialen Testlauf."
  exit 0
fi
...
for d in "${uniq_dirs[@]}"; do
  ...
  # In der echten Version: ./test_all.sh $OPTIONS
  cd ..
  echo ""
done

echo "Führe abschließend das Umbenenn-Skript aus: ./rename_testresult.sh $OPTIONS"
# In der echten Version: ./rename_testresult.sh $OPTIONS
echo "=== TESTFORTSETZUNG ENDE (DRY RUN) ==="
