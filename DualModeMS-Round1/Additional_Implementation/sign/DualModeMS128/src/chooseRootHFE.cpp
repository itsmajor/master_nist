...
*/
int chooseRootHFE(gf2n root, const complete_sparse_monic_gf2nx F, cst_gf2n U)
{
    #if (HFEDeg==1)
        HFEADD(root,F.poly,U);
        return 1U;
    #else
    ...
    /*  F_HFE(x) */
    convHFEpolynomToNTLGF2EX(F_NTL,F.poly);
    ...
    /* X^(2^n) - X mod F */
    ...
    #if 1
        gf2nx poly=(UINT*)calloc(((HFEDeg<<1)-1)*NB_WORD_GFqn,sizeof(UINT));
        if(!poly) 
        {
            clear(monom);
            monomX.kill();
        ...
        frobeniusMap(poly,F,U);
        convPolynomToNTLGF2EX(Xqn,poly);
        free(poly);
        GCD(Pgcd,F_NTL,Xqn);
    #else
        ...
        /* FrobeniusMap */
        Xqn=GF2EX(1UL<<HFEDegI,1);
        for(i=HFEDegI;i<HFEn;++i)
        { 
            SqrMod(Xqn,Xqn,F_NTL_opt);
        }
    ...
    Pgcd.kill();
        ...
        } else
        {
            ...
            UINT roots_UINT [NB_WORD_GFqn*l];
            for(i=0;i<l;++i)
            {
